...
export class ChatService {
  ...
  async run(messages: Message[]): Promise<ChatResponse> {
    ...
    const chatClient = this.chatClient({
      temperature: 0.7, // controls the randomness of the model. A value of 0 will make the model deterministic, and a value of 1 will make it generate the most random answers.
      maxTokens: 1024, // is the maximum number of tokens the model will *generate*. If you set it too low, the model will not be able to generate long answers. If you set it too high, the model may generate answers that are too long.
      n: 1, // number of answers the model will generate
    });
